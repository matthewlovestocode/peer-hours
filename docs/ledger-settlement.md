...
The pure ledger now defaults to a -50-hour (`-3000` minute) minimum balance. It applies ordinary settlements in stable transfer-ID order and excludes an over-limit settlement from derived postings with a visible `minimum-balance` rejection. This is verified in-memory behavior, not yet an end-to-end replicated finality protocol: the desktop does not yet show pending or rejected settlement states, and member feeds do not yet establish how concurrent offline transfers arrive. Disputes, multi-device keys, private contact sharing, and replicated ledger persistence remain deferred.
