...
Peer Hours has a working **signed local workflow and replicated-resolution foundation**, not a production timebank yet. The following pieces are implemented and tested today:
...
Each member runtime owns a separate writable member feed, and a receiving runtime can directly replicate a known member-feed key. A self-certifying identity can sign a declaration linking itself to that feed, and the resolver admits that root key for the member's signed records without a community authorization event. Its feed-aware API also rejects a domain record supplied from a feed the author did not declare. Connected runtimes can exchange root-signed, expiring feed announcements over their shared discovery core; a valid announcement opens the named feed automatically for replication. The desktop safely publishes signed listings, proposals, acceptances, completion acknowledgements, participant-owned settlement attestations, and the deterministic transfer once both attestations are present. The always-on community node is simply another peer with durable storage: it has no human member identity, assists discovery and replication, and has no community record core, member-admission role, or special validity authority. A desktop's locally admitted transfer is not proof of durable replication or social finality. The adopted pilot policy adds one-node and two-independent-node retention-receipt labels without making those nodes accounting authorities; see [pilot operating policy](pilot-operating-policy.md). See [open participation and agreement privacy](open-participation-and-agreement-privacy.md) for the decision that this protocol must not become membership approval.
...
`desktop`, the headless `node`, the `dev-peers` simulator, and the shared `peer-runtime` package now exist. The desktop embeds a local peer runtime; the always-on community peer provides persistent storage, bootstrap metadata, and peer status; and `dev-peers` provides real independent runtimes plus development-only roster registration for UI work. The desktop has a drawer-based application shell, network diagnostics in a separate workspace, and a member workflow for signed listings through locally admitted settlements. The pure `timebank-domain`, `timebank-settlement`, `timebank-ledger`, `timebank-identity`, and `timebank-records` packages define agreement, proposal-to-transfer validation, settlement, key lifecycle, feed provenance, and immutable-record resolution rules. Member runtimes own member feeds; the community peer deliberately does not because it has no human member identity. Durability acknowledgement, recovery, privacy, and pilot operating workflows remain to be built. The admin application should be added when its first concrete workflows are understood.
...
Today, the runtime checks that bootstrap returns a successful HTTP response and structurally validates a complete manifest: nonblank community ID and display name, a positive protocol version, a fixed-length hexadecimal discovery-core key, the narrow `bootstrap` role, a discovery-metadata capability, optional community-node diagnostics URL, HTTP(S) fallback bootstrap URLs, and optional bounded `receiptNodes` metadata. Each receipt node supplies a stable node ID, Ed25519 public key, and receipt URL; duplicate identities or URLs are rejected. This structural validation still does **not** authenticate the endpoint or verify a signed manifest. The pilot trust direction is member-installed discovery and receipt-node pins, with visible review for any identity change; a production bootstrap design still needs a verifiable manifest-distribution and node-rotation process.
