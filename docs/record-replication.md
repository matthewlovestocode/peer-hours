...
The verified tests are protocol vertical slices, and the desktop now exposes the corresponding local workflow: its main process creates a self-owned root identity, protects the private key with operating-system-backed storage, appends the root-signed feed declaration, and signs and publishes listings, proposals, acceptances, and settlement acknowledgements. The renderer presents pending and settled states from locally resolved records. This is still not a claim that another peer received a record, that the counterparty agreed, or that a replicated history has external finality; those are separate observations and policy questions.
