...
The node and bootstrap HTTP services interpret routes by pathname, not by the complete request target, and return JSON with `nosniff`, `no-referrer`, and explicit cache policy headers. The community node keeps diagnostics non-cacheable. Bootstrap metadata alone may be cached briefly (`max-age=60`); it is static convenience metadata, never a member or ledger response. Neither service adds a write surface as part of these operational endpoints.
