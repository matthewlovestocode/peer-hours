...
The node integration suite now exercises a bounded multi-node recovery path: a member-owned feed reaches two independently configured community runtimes, one runtime is stopped, the member appends new history through the survivor, and the stopped runtime restarts and catches up through the survivor. It confirms the code path for discovery relay, restart, and append-only catch-up under deterministic local transport. It does not prove a deployed node's public reachability, physical failure-domain independence, or complete history; operators must still perform and record the equivalent real deployment drill.
