...
This is not the complete trust protocol. The runtime can exchange signed, expiring announcements for declared feeds once peers share a discovery-core scope, but private identity metadata, user-controlled filtering, and multiwriter ordering remain protocol work. Peer Hours has decided against membership approval as the participation gate; a valid self-owned root declaration can authorize member signatures and root-signed device-key rotation/recovery records. The resolver requires matching member-feed provenance before admitting those recovery records, allows active-key overlap, and treats revocation as permanent. Older unsigned authorization lifecycle records remain compatibility-only and cannot modify a member that has self-owned feed provenance. The desktop exposes the current signed workflow through participant settlement attestations and locally admitted deterministic transfers, but it must not claim a replication threshold or irreversible finality until a community policy defines and verifies one.
